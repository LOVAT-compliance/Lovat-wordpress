...
class Lovat_Admin
{
	...
	public static function lovat_settings_page()
	{
		...
		if (!is_null(self::isset_token_by_user($user->ID))) self::add_warning('You have alredy generated a token.  If you will press "Generate key" you will RESET it.');
	...
	public static function save_settings()
	{
		...
		if (!empty($_POST['generate-key'])) {
			if (wc_current_user_has_role('administrator')) {
				...
				if (!is_null($issetUserToken)) {
					...
					self::add_success('Key was updated. New key : ' . $bearerToken);
				} else {
					$wpdb->insert(
						$wpdb->prefix . 'lovat_api_keys',
					...
					self::add_success('Key was generated. Key : ' . $bearerToken);
				}
			} else self::add_error('Only admin can genrate key');
		}
		...
		if (!empty($_POST['save-departure-country'])) {
			...
			self::add_success('Send country saved succesfly');
		}
	}
